...
OPTIMIZED VERSION:
- Multithreaded VideoStream for faster frame reading
...
class DualDetector:
    ...
    def log_detections_to_csv(self, frame_number, pothole_detections,
        ...
        """
        Log detections to internal list for later CSV export
        """
        # Log potholes
        ...
        for idx, detection in enumerate(obstacle_detections):
            ...
            # Get motion status if it's a vehicle
            motion_status = motion_statuses.get(idx, 'N/A')
    ...
    def save_csv(self, output_path):
        """
        Save logged detections to CSV file
        """
        if not self.csv_data:
            print("[INFO] No detections to save to CSV.")
            return

        try:
            with open(output_path, 'w', newline='') as csvfile:
                fieldnames = ['Serial_Number', 'Frame_Number', 'Class',
                              'Confidence', 'BBox', 'Diameter', 'Motion_Status']
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

                writer.writeheader()
                for i, row in enumerate(self.csv_data):
                    # Add serial number
                    row_with_serial = {'Serial_Number': i + 1}
                    row_with_serial.update(row)
                    writer.writerow(row_with_serial)

            print(f"[INFO] CSV file saved: {output_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Dual Detection System - Optimized Version',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 optimized_main.py --pothole-model pothole.onnx --obstacle-model yolo11.pt --video input.mp4 --output result.mp4
...
Optimizations:
  ...
  ✓ Detects stationary vehicles properly
    ...
    # Print configuration
    print("\n" + "="*60)
    print("DUAL DETECTION SYSTEM - OPTIMIZED VERSION")
    print("="*60)
    ...
    print(f"\nFEATURES:")
    print(f"  ✓ Multithreaded VideoStream")
